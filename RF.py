...
def clf_model():
    ...
    #'criterion': 'entropy', 'max_depth': 20, 'max_features': 'sqrt', 'min_samples_leaf': 1,
...
def main():
    ...
    plt.savefig("./ML_Air_Swab_Sum_Win/RF/ROC_curve.pdf", dpi=600)
    plt.show()
    ...
    panda_Mtrcs_test = pd.DataFrame(data = final_score_test, index=grp.tolist(),
                            columns = ["Accuracy","Precision","Recall", "F1score"])
    panda_Mtrcs_test.to_csv('./ML_Air_Swab_Sum_Win/RF/RF_Precision_Recall_F1_test.csv')
