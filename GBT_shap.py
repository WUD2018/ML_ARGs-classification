...
def main():
    ...
    for (g, colorname) in zip(grp, colornames):
        ...
        for (Train, Valid), i in zip(cv.split(X_train, y), range(5)):
            ...
            if acc_t > best:
                best = acc_t
                with open('./ML_Air_Swab_Sum_Win/GBT/best_model_{}.pkl'.format(g), 'wb') as file:
        ...
        with open('./ML_Air_Swab_Sum_Win/GBT/best_model_{}.pkl'.format(g), 'rb') as file:
            model = pickle.load(file)

        def f(x):
            return model.fit(X_train.iloc[Train], y[Train]).predict_proba(x)[:, 0]

        explainer = shap.Explainer(f, X_train)
        shap_values = explainer(X_test)
        fig, ax = plt.subplots()
        shap.summary_plot(shap_values, X_test)
        fig.savefig("./ML_Air_Swab_Sum_Win/GBT/shap_summary_plot_{}.pdf".format(g),
                    dpi=600)  # Saves the plot as a .png file
        plt.close()
